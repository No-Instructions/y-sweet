...
use std::{
    fs::{create_dir_all, remove_file, read_dir},
    path::PathBuf,
    time::SystemTime,
};
...
impl Store for FileSystemStore {
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<FileInfo>> {
        ...
        let dir_entries = match read_dir(&dir_path) {
            Ok(entries) => entries,
            Err(e) => return Err(StoreError::ConnectionError(format!("Failed to read directory: {}", e))),
        };
    ...
    async fn test_list_files() {
        ...
        for (hash, content) in &test_files {
            let found = files.iter().any(|file| {
                file.key == *hash && file.size == content.as_bytes().len() as u64 && file.last_modified > 0
            });
