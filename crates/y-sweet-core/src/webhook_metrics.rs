use prometheus::{
    CounterVec, HistogramVec, GaugeVec, Opts, HistogramOpts, Registry
};
...
impl WebhookMetrics {
    pub fn new() -> Result<Arc<Self>, prometheus::Error> {
        match WEBHOOK_METRICS.get_or_init(|| {
            Self::new_with_registry(prometheus::default_registry())
        }) {
    ...
    pub fn record_webhook_request(&self, prefix: &str, doc_id: &str, status: &str, duration_seconds: f64) {
...
impl Default for WebhookMetrics {
    fn default() -> Self {
        Self::new().expect("Failed to create webhook metrics").as_ref().clone()
    }
}